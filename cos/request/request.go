...
func New(cfg cos.Config, clientInfo metadata.ClientInfo, handlers Handlers,
	...
	var err error
    // TODO(LC): ADD APPID IN URL, must be fix
	httpReq.URL, err = url.Parse(clientInfo.Endpoint + operation.HTTPPath)
	if err != nil {
		httpReq.URL = &url.URL{}
		err = coserr.New("InvalidEndpointURL", "invalid endpoint uri", err)
	}
...
func (r *Request) WillRetry() bool {
	return r.Error != nil && cos.BoolValue(r.Retryable) && r.RetryCount < r.MaxRetries()
}
...
func debugLogReqError(r *Request, stage string, retrying bool, err error) {
	if !r.Config.LogLevel.Matches(cos.LogDebugWithRequestErrors) {
		return
	}
...
func (r *Request) Send() error {
	for {
		if cos.BoolValue(r.Retryable) {
			if r.Config.LogLevel.Matches(cos.LogDebugWithRequestRetries) {
